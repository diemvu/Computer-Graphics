...
//			System.out.println("Bad y: " + y);
	...
	void drawTri(double x1, double y1, double z1, double x2, double y2, double z2, double x3, double y3, double z3,
			int r, int g, int b) {
	...
	void drawLine(double x1, double y1, double z1, double x2, double y2, double z2, int r, int g, int b) {
	...
	Vector[] cubeVert = { new Vector(-0.5, 0.5, 0.5, 1), // front top left 0
	...
	void createImage() {
		...
		Matrix viewerXmatr = new Matrix(new Vector(1, 0, 0, 0),
		...
		while (input.hasNext()) {
			...
			if (command.equals("DIM")) {
				...
				lightx = input.nextFloat();
				lighty = input.nextFloat();
				lightz = input.nextFloat();
				lightDirection = new Vector(lightx, lighty, lightz, 1);
...
//				System.out.println("left" + left + "right" + right + "top" + top + "bottom" + bottom + "near" + near
//						+ "far" + far);

				perspective = new Matrix(new Vector(2 / (right - left), 0, 0, ((right + left) * (-1)) / (right - left)),
						new Vector(0, 2 / (top - bottom), 0, ((top + bottom) * (-1)) / (top - bottom)),
						new Vector(0, 0, (-2) / (far - near), ((far + near) * (-1)) / (far - near)),
						new Vector(0, 0, 0, 1));
...
//						+ "far" + far);
				perspective = new Matrix(
						new Vector((2 * near) / (right - left), 0, 0, ((right + left) * near * (-1)) / (right - left)),
						new Vector(0, (2 * near) / (top - bottom), 0, ((top + bottom) * near * (-1)) / (top - bottom)),
						new Vector(0, 0, ((far + near) * (-1)) / (far - near), (2 * far * near) / (far - near)),
						new Vector(0, 0, -1, 0));
				...
				drawLine(v1.x / v1.w, v1.y / v1.w, v1.z / v1.w, v2.x / v2.w, v2.y / v2.w, v2.z / v2.w, r, g, b);
				...
				Vector normal = Vector.normalVector(v1, v2, v3);
				double shading = Vector.getCosAngle(normal, lightDirection);
				...
				int newR = getNewColor(r, shading);
				int newG = getNewColor(g, shading);
				int newB = getNewColor(b, shading);

				drawTri(v1.x / v1.w, v1.y / v1.w, v1.z / v1.w, v2.x / v2.w, v2.y / v2.w, v2.z / v2.w, v3.x / v3.w,
						v3.y / v3.w, v3.z / v3.w, newR, newG, newB);

			} else if (command.equals("LOAD_IDENTITY_MATRIX")) {
				ctm = Matrix.IDENTITY;

			} else if (command.equals("SCALE")) {
				Matrix trans = new Matrix(new Vector(input.nextFloat(), 0, 0, 0),
						new Vector(0, input.nextFloat(), 0, 0), new Vector(0, 0, input.nextFloat(), 0),
						new Vector(0, 0, 0, 1));
				ctm = Matrix.multiply(trans, ctm);

			} else if (command.equals("ROTATEX")) {
				double th = (2 * Math.PI * input.nextFloat()) / 360;
				Matrix rotx = new Matrix(new Vector(1, 0, 0, 0), new Vector(0, Math.cos(th), -Math.sin(th), 0),
						new Vector(0, Math.sin(th), Math.cos(th), 0), new Vector(0, 0, 0, 1));
				ctm = Matrix.multiply(rotx, ctm);

			} else if (command.equals("ROTATEY")) {
				double th = (2 * Math.PI * input.nextFloat()) / 360;
				Matrix roty = new Matrix(new Vector(Math.cos(th), 0, Math.sin(th), 0), new Vector(0, 1, 0, 0),
						new Vector(-Math.sin(th), 0, Math.cos(th), 0), new Vector(0, 0, 0, 1));
				ctm = Matrix.multiply(roty, ctm);

			} else if (command.equals("ROTATEZ")) {
				double th = (2 * Math.PI * input.nextFloat()) / 360;
				Matrix rotz = new Matrix(new Vector(Math.cos(th), -Math.sin(th), 0, 0),
						new Vector(Math.sin(th), Math.cos(th), 0, 0), new Vector(0, 0, 1, 0), new Vector(0, 0, 0, 1));
				ctm = Matrix.multiply(rotz, ctm);

			} else if (command.equals("TRANSLATE")) {
				Matrix trans = new Matrix(new Vector(1, 0, 0, input.nextFloat()),
						new Vector(0, 1, 0, input.nextFloat()), new Vector(0, 0, 1, input.nextFloat()),
						new Vector(0, 0, 0, 1));
				ctm = Matrix.multiply(trans, ctm);
				...
				for (int i = 0; i < 12; i++) {
					...
					drawLine(pt1.x / pt1.w, pt1.y / pt1.w, pt1.z / pt1.w, pt2.x / pt2.w, pt2.y / pt2.w, pt2.z / pt2.w,
							r, g, b);
				}
	...
	public int getNewColor(int color, double shading) {
		return (int) Math.round(color * 0.5 + 0.5 * shading * color);
	}

	public Matrix lookAt(Vector eye, Vector at, Vector up) {
		Vector dir = Vector.subtract(eye, at);
		dir = Vector.normalize(dir);
		Vector m = Vector.crossProduct(up, dir);
		m = Vector.normalize(m);
		Vector n = Vector.crossProduct(dir, m);
		n = Vector.normalize(n);

		double tx = m.dotProduct(eye) * (-1);
		double ty = n.dotProduct(eye) * (-1);
		double tz = dir.dotProduct(eye) * (-1);

		return new Matrix(new Vector(m.x, m.y, m.z, tx), new Vector(n.x, n.y, n.z, ty),
				new Vector(dir.x, dir.y, dir.z, tz), new Vector(0, 0, 0, 1));
	...
	public int[] newPixels() {
		int nht = height / 2;
		int nwd = width / 2;
		int[] newPixels = new int[nwd * nht * 3];
		for (int row = 0; row < nht; row++) {
			for (int col = 0; col < nwd; col++) {
				int pixelIndex = (row * 2) * width + col * 2;
				int oldIndex1 = pixelIndex * 3;
				int oldIndex2 = (pixelIndex + 1) * 3;
				int oldIndex3 = (pixelIndex + width) * 3;
				int oldIndex4 = (pixelIndex + width + 1) * 3;
				newPixels[row * nwd * 3
						+ col * 3] = (pixels[oldIndex1] + pixels[oldIndex2] + pixels[oldIndex3] + pixels[oldIndex4])
								/ 4;
				newPixels[row * nwd * 3 + col * 3 + 1] = (pixels[oldIndex1 + 1] + pixels[oldIndex2 + 1]
						+ pixels[oldIndex3 + 1] + pixels[oldIndex4 + 1]) / 4;
				newPixels[row * nwd * 3 + col * 3 + 2] = (pixels[oldIndex1 + 2] + pixels[oldIndex2 + 2]
						+ pixels[oldIndex3 + 2] + pixels[oldIndex4 + 2]) / 4;
	...
	public void paintComponent(Graphics g) {
		...
		wrraster.setPixels(0, 0, width / 2, height / 2, pixels);
