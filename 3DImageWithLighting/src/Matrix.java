/**
 * This class represents a 4x4 matrix to be used in graphics transformations. It
 * is stored as an array of Vectors with each Vector representing a row of the
 * matrix.
 *
 */
public class Matrix {
	...
	public static final Matrix IDENTITY = new Matrix(new Vector(1, 0, 0, 0), new Vector(0, 1, 0, 0),
			new Vector(0, 0, 1, 0), new Vector(0, 0, 0, 1));

	/**
	 * Public constructor that makes a matrix from four given vectors. Each vector
	 * is a row of the matrix starting from the top.
	 * 
	...
	/**
	 * Returns a column from the matrix. Constructs a vector from the four values in
	 * the columns. The vector values (x, y, z, w) are obtained from the matrix
	 * starting at row1 down. Used for matrix multiplication.
	 * 
	 * @param col column of the matrix
	 * @return Vector constructed from the numbers in the given column
	 */
	...
	/**
	 * Overloaded function for matrix multiplication. This version takes two
	 * matrices as arguments, multiplies the left one by the right one, and returns
	...
	/**
	 * Overloaded function for matrix multiplication. This version takes a matrix
	 * and a vector, multiplies the two and returns a new vector containing the
	 * result.
	 * 
